...
public class PathFinder : MonoBehaviour {
	...
	bool lineOfSight(Vector3 origin, Vector3 destination){
	...
	public Vector3 getPath(Vector3 origin, Vector3 destination){
		if(lineOfSight(origin, destination))
		{
			return destination;
		}
		if(goal==null || goal.position!=destination)
			buildStartAndGoal(origin, destination);
		checkWayPointVision();
		//we are at end of path (i.e. goal)
		if(currentPathIndex==path.Count){
			return destination;
		}
		else{
			if(Vector3.Distance(origin, path[currentPathIndex].position)<threshhold)
				currentPathIndex++;
			if(currentPathIndex==path.Count)
				return destination;
			return path[currentPathIndex].position;
		}
	}
}
